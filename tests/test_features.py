...
from ubs_recurrence.augmentation import mask_descriptions,corrupt_transactions
...
def history():
    rows=[]
    for cid,base,desc,mcc in [("A",7,"cloud access","5732"),("B",60,"phone contract","4814")]:
        for n in range(5):
            rows.append({"client_id":cid,"timestamp":CUTOFF-pd.Timedelta(days=10+30*n),"amount":base*(1+n*.001),"description":desc,"mcc":mcc,"currency":"chf","type":"card_payment","direction":"out","fee":0.0})
...
def test_masking_preserves_nontext_fields_and_is_order_invariant():
    d=history();a=mask_descriptions(d,.5,42);b=mask_descriptions(d.sample(frac=1,random_state=12),.5,42)
    pd.testing.assert_frame_equal(a,b)
    pd.testing.assert_frame_equal(a.drop(columns="description"),d.reset_index(drop=True).drop(columns="description"))
    c=corrupt_transactions(d,"test_like",42)
    for col in ["client_id","timestamp","amount","direction","type","currency","fee"]:
...
def test_features_have_no_identifier_dependence_and_include_every_candidate():
    d=history();ids=np.array(["A","B"])
    T=template_features(d,ids);S=extract_streams(d,filter_background=True);F=family_features(S,ids);X=ranking_features(F,T,ids,clocks=False)
    assert X.shape[0]==16
    ...
    np.testing.assert_allclose(T.to_numpy(),template_features(d.sample(frac=1,random_state=7),ids).to_numpy())


def test_input_label_columns_do_not_affect_features():
    d=history();ids=np.array(["A","B"])
    with_label=d.assign(target_next_recurring_merchant="fake")
    pd.testing.assert_frame_equal(template_features(d,ids),template_features(with_label,ids))
