...
from ubs_recurrence.data import PREDICTION, labels, validate_submission
...
def test_metric_uses_all_eight_classes():
    ...
    assert metrics(y, p)["accuracy"] == 0.125
...
def test_submission_contract():
    ...
    for bad in [
        sample.iloc[:1],
        pd.concat([sample, sample.iloc[:1]]),
        sample.assign(**{PREDICTION: ["fake", "none"]}),
        sample.assign(client_id=["a", "c"]),
        sample.rename(columns={PREDICTION: "wrong"}),
    ]:
        with pytest.raises(ValueError):
            validate_submission(bad, sample)
...
def test_normalization_is_deterministic():
    ...
    assert normalize(normalize("Pay cloud BACKUP 123")) == normalize(
        "Pay cloud BACKUP 123"
    )


def test_probabilities_rejected_if_invalid():
    with pytest.raises(ValueError):
        metrics(np.arange(8), np.ones((8, 8)))
