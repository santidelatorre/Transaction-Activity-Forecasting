...
def test_illegal_and_duplicate_labels_are_rejected(tmp_path, monkeypatch):
    ...
    for ids, labels in [
        ...
        pd.DataFrame(
            {"client_id": ids, "cutoff_date": ["2026-01-01"] * 2, data.TARGET: labels}
        ).to_csv(raw / "train_labels.csv", index=False)
        with pytest.raises(ValueError):
            data.labels()
...
def test_post_cutoff_transaction_is_rejected(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="cutoff"):
        data.transactions("train", use_cache=False)
