...
const families = { cloud: 'servicios en la nube', gym: 'gimnasio', insurance: 'seguros', mobile: 'telefonía', music: 'música', software: 'software', streaming: 'streaming' };
...
function featureName(name) {
  ...
  if (match) return `Operaciones diarias medias en ${match[1]} · ${match[2]} días`;
...
function useResource(path, revision) {
  ...
  useEffect(() => {
    ...
    fetch(path, { signal: controller.signal, headers: { Accept: 'application/json' } })
      .then(async (response) => {
        if (!response.ok) throw new Error(`No hemos podido consultar los resultados (${response.status}).`);
        return response.json();
      })
      .then((data) => setState({ data, loading: false, error: null }))
      .catch((error) => {
        if (error.name !== 'AbortError' || timedOut) setState({ data: null, loading: false, error: timedOut ? 'La consulta ha tardado demasiado. Vuelve a intentarlo.' : error.message });
      })
...
function Empty({ error, loading, children }) {
  return <div role={error ? 'alert' : 'status'} className="flex min-h-48 flex-col items-center justify-center px-6 py-10 text-center">
    ...
    <p className="max-w-sm text-sm leading-6 text-[#686868]">{loading ? 'Consultando los resultados del equipo…' : error || children}</p>
  </div>;
}
...
function ProgressChart({ data, loading, error }) {
  ...
  return <section id="evolucion" className="scroll-mt-24 rounded-sm border border-[#E4E4E4] bg-white">
    <div className="px-6 pt-6 sm:px-7">
      <Eyebrow className="text-[#858585]">01 / El camino del modelo</Eyebrow>
      <h2 className="mt-2 text-xl font-medium tracking-tight">Cada iteración, a la vista.</h2>
      <p className="mt-2 text-xs leading-5 text-[#686868]">Macro-F1 de los candidatos sobre el mismo conjunto VALID.</p>
    </div>
    {loading || error || !rows.length ? <Empty loading={loading} error={error}>El gráfico aparecerá cuando haya evaluaciones registradas. La meta del equipo es 0,80.</Empty> : <>
      <div className="px-4 pt-4 sm:px-6">
        <div ref={chartContainer}>
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full overflow-visible" aria-labelledby={`${chartId}-title ${chartId}-desc`}>
          <title id={`${chartId}-title`}>Comparación de Macro-F1 con objetivo 0,80</title>
          <desc id={`${chartId}-desc`}>Puntos ordenados según el informe de validación, sin implicar cronología. El modelo de referencia es V3-A. Los valores exactos están en la tabla de experimentos.</desc>
          ...
          <text x={width - right} y={y(0.8) - 9} textAnchor="end" fill={RED} fontSize="10" letterSpacing="1">META DEL EQUIPO · 0,80</text>
      ...
      <p className="px-6 py-4 text-[11px] leading-5 text-[#757575] sm:px-7">Orden del informe, no cronología. VALID se ha reutilizado para comparar modelos; esta curva no mide el rendimiento en TEST.</p>
    </>}
  </section>;
}
...
function ImportanceChart({ data, loading, error }) {
  ...
  return <section id="explicabilidad" className="scroll-mt-24 rounded-sm border border-[#E4E4E4] bg-white">
    <div className="px-6 pt-6 sm:px-7">
      <Eyebrow className="text-[#858585]">02 / Las señales detrás de la decisión</Eyebrow>
    ...
    {loading || error || !rows.length ? <Empty loading={loading} error={error}>Todavía no hay importancias medidas para este modelo. Se mostrarán aquí cuando estén disponibles.</Empty> : <>
      ...
      <details className="mx-6 border-t border-[#EAEAEA] py-3 sm:mx-7">
        ...
        <p className="mt-3 text-[11px] leading-5 text-[#686868]">{data.importance_scope}. Estas contribuciones globales no explican una predicción individual. La asociación de una descripción a una familia es inferida.</p>
...
function ExperimentTable({ revision }) {
  ...
  return <section id="experimentos" className="mt-6 scroll-mt-24 rounded-sm border border-[#E4E4E4] bg-white">
    <div className="flex flex-wrap items-end justify-between gap-4 px-6 py-6 sm:px-7">
      <div><Eyebrow className="text-[#858585]">03 / Resultados que se pueden comprobar</Eyebrow><h2 className="mt-2 text-xl font-medium tracking-tight">La evidencia de cada decisión.</h2></div>
    ...
    {loading || error || !rows.length ? <Empty loading={loading} error={error}>No hay experimentos en esta página. Los resultados aparecerán al cargar el registro del equipo.</Empty> : <div className="overflow-x-auto">
      <table className="w-full min-w-[720px] text-left text-xs">
        <caption className="sr-only">Experimentos reales. Macro-F1 y porcentaje de aciertos en su conjunto de evaluación. Pulsa una fila para consultar la procedencia.</caption>
        <thead className="border-y border-[#E4E4E4] bg-[#F5F5F5] text-[10px] font-medium uppercase tracking-[0.09em] text-[#757575]">
          <tr><th scope="col" className="px-6 py-3.5 sm:pl-7">Experimento / modelo</th><th scope="col" className="px-4 py-3.5">Evaluación</th><th scope="col" className="px-4 py-3.5 text-right">Macro-F1</th><th scope="col" className="px-4 py-3.5 text-right">Acierto global</th><th scope="col" className="px-4 py-3.5 text-right">Δ vs base</th><th scope="col" className="px-6 py-3.5 text-right sm:pr-7">Estado</th></tr>
        </thead>
    ...
    <div className="flex items-center justify-between gap-4 border-t border-[#E4E4E4] px-6 py-4 text-[11px] text-[#686868] sm:px-7">
      <span>Página {page + 1}<span className="hidden sm:inline"> · Δ respecto a la base de cada registro</span></span>
...
function ExperimentRows({ row, index, expanded, toggle }) {
  ...
  return <>
    <tr className={`border-t border-[#EEEEEE] ${index % 2 ? 'bg-[#FAFAFA]' : 'bg-white'}`}>
      ...
      <td className="px-6 py-4 text-right sm:pr-7"><span className={`inline-block whitespace-nowrap px-2 py-1 text-[10px] ${row.result === 'selected' ? 'border border-[#FFD1D1] bg-[#FFF6F6] text-[#C90000]' : 'text-[#757575]'}`}>{row.result === 'selected' ? 'Modelo de referencia' : 'Evaluado'}</span></td>
    </tr>
...
export default function App() {
  ...
  return <div className="min-h-screen bg-white font-sans text-[#1A1A1A] selection:bg-[#FFE2E2]">
    <a href="#contenido" className="sr-only z-50 bg-white p-4 focus:not-sr-only focus:fixed">Saltar al contenido</a>
    <header className="sticky top-0 z-30 border-b border-[#EAEAEA] bg-white/95 backdrop-blur-sm">
      <div className="mx-auto flex h-[76px] max-w-[1320px] items-center justify-between gap-5 px-5 sm:px-8 lg:px-12">
        <a href="#contenido" className="flex shrink-0 items-center gap-3" aria-label="Recurring Insights, inicio">
        ...
        <nav aria-label="Navegación principal" className="hidden items-center gap-7 text-xs text-[#686868] md:flex">
          <a href="#calidad" className="hover:text-[#E60000]">Visión general</a><a href="#explicabilidad" className="hover:text-[#E60000]">Cómo decide</a><a href="#experimentos" className="hover:text-[#E60000]">Experimentos</a>
        </nav>
    ...
    <main id="contenido" className="mx-auto max-w-[1320px] scroll-mt-24 px-5 pb-14 sm:px-8 lg:px-12">
      <section className="grid gap-10 pb-10 pt-10 sm:pt-14 lg:grid-cols-[1fr_260px] lg:items-center lg:gap-20">
        <div>
          ...
          <a href="#explicabilidad" className="mt-6 inline-flex items-center gap-3 text-xs font-semibold text-[#E60000]">Explorar la evidencia <ArrowDown size={14} /></a>
        </div>
      ...
      <section id="calidad" className="scroll-mt-24 border-t border-[#EAEAEA] pt-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <Eyebrow className="text-[#757575]">Lo que hemos demostrado</Eyebrow>
          <span className="flex items-center gap-2 text-[11px] text-[#686868]"><span className={`h-1.5 w-1.5 rounded-full ${ready ? 'bg-[#1A1A1A]' : 'bg-[#B8B8B8]'}`} />{loading ? 'Consultando evaluación' : ready ? `${data.model_version} · evaluación VALID` : 'Evaluación pendiente de cargar'}</span>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <MetricCard primary number="01" title="Calidad de predicción general" value={format(metrics?.macro_f1, 4)} annotation="Macro-F1 · cada una de las ocho clases tiene el mismo peso.">
            ...
            <span className="mt-1 block">Meta del equipo: ≥ 0,80 · aspiración, no resultado.</span>
          </MetricCard>
          <MetricCard number="02" title="Predicciones acertadas" value={percent(metrics?.accuracy)} annotation="Acierto global (accuracy): coincidencias exactas sobre el total de clientes.">
            <span>Medido en VALID · resultado de TEST no disponible.</span>
          </MetricCard>
          <MetricCard number="03" title="Clientes que respaldan el resultado" value={metrics?.validation_clients?.toLocaleString('de-DE') ?? '—'} annotation="Clientes del conjunto de validación, contados a partir de la matriz de confusión.">
      ...
      <section id="metodo" className="mt-10 grid gap-7 border-y border-[#E4E4E4] py-8 md:grid-cols-[1.1fr_1fr_1fr]">
        ...
        <div><h3 className="text-xs font-semibold">La validación tiene un alcance</h3><p className="mt-2 text-xs leading-6 text-[#686868]">VALID se ha reutilizado durante la comparación de candidatos. «None» significa que no se predice una familia recurrente en el horizonte; no garantiza la ausencia de movimientos.</p></div>
      </section>
      <details className="group border-b border-[#E4E4E4] py-4 text-xs text-[#686868]">
        ...
        <dl className="mt-4 grid gap-4 text-[11px] sm:grid-cols-2">
          <div><dt className="font-medium text-[#1A1A1A]">Modelo de referencia</dt><dd className="mt-1">{data?.model_version || 'Pendiente'} · horizonte de 90 días</dd></div>
