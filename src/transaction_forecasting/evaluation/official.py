...
def _check_ids(frame: pd.DataFrame, name: str) -> None:
    ...
    ids = frame["client_id"]
    if not ids.map(lambda v: isinstance(v, str) and bool(v.strip()) and v == v.strip()).all():
        raise ValueError(f"{name}: client IDs must be nonempty strings without outer whitespace")
...
def validate_submission(predictions: pd.DataFrame, sample: pd.DataFrame) -> pd.DataFrame:
...
def score_predictions(labels: pd.DataFrame, predictions: pd.DataFrame) -> dict[str, object]:
    ...
    confusion = {
        actual: {guess: int((truth.eq(actual) & predicted.eq(guess)).sum()) for guess in LABELS}
...
def main(argv: list[str] | None = None) -> int:
    ...
    score = commands.add_parser("score", help="Score validation predictions against labels")
    score.add_argument("--labels", type=Path, required=True)
    score.add_argument("--predictions", type=Path, required=True)
    check = commands.add_parser("validate", help="Check test submission against sample IDs")
