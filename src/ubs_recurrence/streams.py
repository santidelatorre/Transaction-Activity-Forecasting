...
from .features import GENERIC, MCC, PATTERNS, normalize
from .templates import TEMPLATES

FAMILIES = LABELS[:-1]


def amount_components(amounts, eps=0.035, min_samples=3):
    """1-D DBSCAN for min_samples 2 or 3 without estimator setup overhead."""
    if min_samples not in {2, 3}:
        raise ValueError("Only min_samples=2 or 3 is supported")
...
def fast_stats(amount, days, desc, mcc, semantic, dom, dow, currency):
    ...
    r = {
        ...
        "amount_mean": a.mean(),
        "amount_std": a.std(),
        "amount_cv": a.std() / a.mean(),
        "amount_min": a.min(),
        "amount_max": a.max(),
        "amount_median": np.median(a),
        "amount_mad": np.median(np.abs(a - np.median(a))) / np.median(a),
        "first_age": -t.min(),
        "last_age": -t.max(),
        "span": np.ptp(t),
        "unique_description": len(counts),
        "description_top_fraction": counts.max() / n,
        "description_entropy": float(-np.sum(counts / n * np.log(counts / n))),
        "unique_mcc": len(unique_mcc),
        "mcc_top_fraction": unique_mcc.max() / n,
        "generic_fraction": np.isin(desc, GENERIC).mean(),
        "dom_std": np.std(dom),
        "dow_std": np.std(dow),
        "currency_count": len(set(currency)),
    }
    for w in [30, 60, 90, 180]:
        r[f"count_{w}"] = int((t >= -w).sum())
    ...
    if len(gap):
        ...
        r.update(
            {
                ...
                "gap_min": gap.min(),
                "gap_max": gap.max(),
                "gap_cv": gap.std() / max(gap.mean(), 1e-6),
                "gap_last": gap[-1],
                "next_median": t[-1] + med,
                "next_mean": t[-1] + gap.mean(),
                "next_last": t[-1] + gap[-1],
                "overdue_ratio": -t[-1] / max(med, 1),
                "gap_recent": np.median(gap[-3:]),
                "next_recent": t[-1] + np.median(gap[-3:]),
                "amount_change": a[-1] / a[0] - 1,
            }
        )
        ...
        for period in [7, 14, 28, 30, 30.4375, 31, 60, 90, 365]:
            ...
            r[f"phase_next_{period}"] = np.mod(
                np.angle(phase) * period / (2 * np.pi), period
            )
        ...
        r["linear_residual"] = np.sqrt(
            np.mean((t - (intercept + slope * np.arange(n))) ** 2)
        )
        r["linear_next"] = intercept + slope * n
    return r


def extract_streams(df, eps=0.035, filter_background=False, min_count=3):
    rows = []
    ...
    semantic = np.stack(
        [
            d.description.str.contains(p.replace("(", "(?:"), regex=True).to_numpy()
    ...
    desc = d.description.to_numpy()
    mcc = d.mcc.to_numpy()
    curr = d.currency.to_numpy()
    dom = d.timestamp.dt.day.to_numpy()
    dow = d.timestamp.dt.dayofweek.to_numpy()
    for (cid, currency), g in d.groupby(["client_id", "currency"], sort=True):
        ix = g.row_number.to_numpy()
        groups = [
            ("amount", ix[c])
            for c in amount_components(amounts[ix], eps, min_samples=min_count)
        ]
        # Broader family/MCC grouping retains variable-amount recurring streams.
        for f in FAMILIES:
            choose = semantic[ix, FAMILIES.index(f)] | (
                (mcc[ix] == MCC[f]) & np.isin(desc[ix], TEMPLATES[f] + GENERIC)
            )
            if choose.sum() >= 2:
                groups.append(("family_" + f, ix[choose]))
        for group_type, index in groups:
            ...
            score = np.array(
                [r[f"semantic_{f}"] + 0.25 * r[f"mcc_{f}"] for f in FAMILIES]
            )
            r.update(
                {
                    "client_id": cid,
                    ...
                    "currency": currency,
                    "family": FAMILIES[int(score.argmax())],
...
def family_features(streams, ids, price_profiles=None):
    ...
    all_numeric = [
        c
        for c in st.select_dtypes(include=np.number).columns
        if not any(s in c for s in ["semantic_", "mcc_", "template_"])
        and c not in ["eps", "family_score"]
    ]
        ...
        for f in FAMILIES:
            ...
            if price_profiles is not None:
                ...
                likelihood = np.stack(
                    [
                        (
                            3 * g["semantic_" + o].to_numpy()
                            + g["mcc_" + o].to_numpy()
                            + 0.05
                        )
                        * price_support(g.amount_median.to_numpy(), price_profiles[o])
                        for o in FAMILIES
                    ],
                    axis=1,
                )
                own = likelihood[:, FAMILIES.index(f)]
                keep = (own >= 0.4 * likelihood.max(axis=1)) & (own > 0.02)
                eligible = g[
                    (keep & (g.group_type == "amount"))
                    | (g.group_type == "family_" + f)
                ].copy()
                row["prior_low"] = price_profiles[f]["low"]
                row["prior_high"] = price_profiles[f]["high"]
            else:
                eligible = g[
                    ((g.family == f) & (g.group_type == "amount"))
                    | (g.group_type == "family_" + f)
                ].copy()
            row["stream_count"] = len(eligible)
            for prefix, sub in [
                ("amount", eligible[eligible.group_type == "amount"]),
                ("broad", eligible[eligible.group_type == "family_" + f]),
            ]:
                sub = sub.copy()
                sub["strength"] = (
                    sub["count"] * np.exp(-sub.last_age / 60) if len(sub) else 0
                )
                ...
                for rank in range(2):
                    ...
                    if s is not None:
                        ...
                        row[f"{prefix}{rank}_own_template_diversity"] = s[
                            f"template_{f}_diversity"
                        ]
                        row[f"{prefix}{rank}_own_template_count"] = s[
                            f"template_{f}_count"
                        ]
                        row[f"{prefix}{rank}_other_semantic"] = max(
                            s["semantic_" + o] for o in FAMILIES if o != f
                        )
                        if price_profiles is not None:
                            row[f"{prefix}{rank}_price_support"] = price_support(
                                s["amount_median"], price_profiles[f]
                            )
                            row[f"{prefix}{rank}_normalized_price"] = (
                                s["amount_median"] - price_profiles[f]["low"]
                            ) / (price_profiles[f]["high"] - price_profiles[f]["low"])
            rows.append(row)
    ...
    for c in [
        ...
        "amount0_last_age",
        "amount0_count",
        "amount0_own_template_diversity",
        "broad0_next_median",
        "broad0_count",
        "broad0_own_template_diversity",
    ]:
        if c in x:
            value = x[c].replace(-999, np.nan)
            x[c + "_rank"] = (
                value.groupby(level=0)
                .rank(
                    method="min", ascending=("count" not in c and "diversity" not in c)
                )
                .fillna(8)
            )
            x[c + "_vs_best"] = value - value.groupby(level=0).transform(
                "min" if "count" not in c and "diversity" not in c else "max"
            )
    return x.fillna(-999)
