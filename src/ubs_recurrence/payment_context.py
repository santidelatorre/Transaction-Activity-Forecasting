...
def payment_context(df, ranking):
    ...
    for index, row in ranking.iterrows():
        ...
        g = grouped[cid]
        result = {}
        card = g[g.type == "card_payment"]
        refund = g[g.type == "refund"]
        for prefix in ["amount0", "broad0"]:
            mean = row.get(prefix + "_amount_median", -999)
            last = row.get(prefix + "_last_age", -999)
            count = row.get(prefix + "_count", -999)
            if mean <= 0 or count < 2:
                continue
            closest = card.iloc[int(np.argmin(np.abs(card.age.to_numpy() - last)))]
            currency = closest.currency
            c = card[
                (card.currency == currency)
                & (np.abs(np.log(card.amount / mean)) < 0.04)
            ]
            r = refund[
                (refund.currency == currency)
                & (np.abs(np.log(refund.amount / mean)) < 0.04)
            ]
            ...
            result[prefix + "_refund_after_last"] = int(
                ((last - r.age).between(0, 10)).any()
            )
            ...
            if len(c):
                ...
                result[prefix + "_weekend_fraction"] = (
                    c.timestamp.dt.dayofweek >= 5
                ).mean()
