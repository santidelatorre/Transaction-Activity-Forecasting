...
def ranking_features(family, templates, ids, clocks=True):
    ...
    for f in LABELS:
        ...
        for scope in ["any","mcc"]:
            for w in [999,90]:
                ...
                counts=templates[[base+t for t in TEMPLATES[f]]].reindex(ids).to_numpy()
                ...
                for stat in ["diversity","repeated","sum","max"]:
                    X.loc[ix,f"own_template_{scope}_{w}_{stat}"]=templates[base+stat].reindex(ids).to_numpy()
    # Global evidence is identical in all eight rows for a client and cannot encode labels.
    for col in [c for c in templates if c.startswith("none__") and not any(t in c for t in ["amount","age"])]:
    ...
    if clocks:
        ...
        for prefix in ["amount0","amount1","broad0","broad1"]:
            if prefix+"_gap_median" not in X:continue
            med=X[prefix+"_gap_median"].to_numpy()
            strength=np.stack([X[f"{prefix}_phase_strength_{p:g}"].to_numpy() if f"{prefix}_phase_strength_{p:g}" in X else X[f"{prefix}_phase_strength_{p}"].to_numpy() for p in periods],axis=1)
            # Prefer clocks explaining both phase and observed event density.
            fit=(1-strength)+.8*np.abs(np.log(np.maximum(med[:,None],1)/periods[None,:]))
            best=fit.argmin(axis=1);period=periods[best]
            phase=np.stack([X[f"{prefix}_phase_next_{p:g}"].to_numpy() if f"{prefix}_phase_next_{p:g}" in X else X[f"{prefix}_phase_next_{p}"].to_numpy() for p in periods],axis=1)[np.arange(len(X)),best]
            ...
            X[prefix+"_clock_strength"]=np.where(valid,strength[np.arange(len(X)),best],0)
            X[prefix+"_active"]=active.astype(int)
            X[prefix+"_next_modulo"]=np.where(valid,np.mod(X[prefix+"_next_median"].to_numpy(),period),999)
    compare=[c for c in X if any(t in c for t in ["clock_active_next","next_modulo","last_age","own_template_mcc_999","_active"]) and not c.endswith(("_rank","_vs_best"))]
    compare += [c for c in ["amount0_count","amount0_gap_cv","amount0_linear_next","broad0_next_median","broad0_count"] if c in X]
    extra={}
    for c in compare:
