...
def mask_descriptions(df, probability, seed):
    ...
    out = (
        df.sort_values(["client_id", "timestamp"], kind="stable")
        .reset_index(drop=True)
        .copy()
    )
    norm = out.description.map(normalize)
    eligible = (
        (out.type.isin(["card_payment", "refund"]))
        & ~norm.str.contains(BACKGROUND, regex=True)
        & ~norm.isin(GENERIC)
    )
...
def corrupt_transactions(df, severity, seed):
    ...
    """
    if severity not in {"valid_like", "test_like"}:
        raise ValueError(severity)
    rates = {
        "valid_like": (0.30, 0.18, 0.06, 0.15),
        "test_like": (0.50, 0.30, 0.10, 0.25),
    }
    ...
    for source, options in confusion.items():
        ix = np.flatnonzero(
            recurring & (original_mcc == source) & (rng.random(len(out)) < mcc_rate)
        )
        mcc[ix] = rng.choice(options, size=len(ix))
    out["mcc"] = mcc
    merchant_templates = sorted(
        {v for f, vals in TEMPLATES.items() if f != "none" for v in vals}
    )
