...
MCC = {"cloud":"5732","gym":"7997","insurance":"6300","mobile":"4814","music":"5812","software":"5734","streaming":"5812"}
GENERIC = ["monthly plan","digital service","member plan","subscription charge"]


def normalize(text):
    text = text.lower()
    for a,b in [("dgtl","digital"),("mth","monthly"),("prem ","premium "),("insur ","insurance ")]:
...
def documents(df, ids, kind="raw"):
    ...
    if kind == "enriched":
        d["description"] = d.description.map(normalize)+" mcc"+d.mcc+" type"+d.type+" " + d.description.map(normalize).str.replace(" ","_") + "_m"+d.mcc
    return d.groupby("client_id").description.agg(" ; ".join).reindex(ids).fillna("").to_numpy()
...
def summarize(g):
    ...
    result = {"count":len(g),"amount_mean":np.mean(a),"amount_std":np.std(a),"amount_min":np.min(a),"amount_max":np.max(a),"amount_median":np.median(a),
              "amount_cv":np.std(a)/max(np.mean(a),1e-6),"first_age":-days.min(),"last_age":-days.max(),"span":np.ptp(days),
              "unique_description":len(desc),"description_top_fraction":desc.iloc[0]/len(g),
              "description_entropy":float(-np.sum((desc/len(g))*np.log(desc/len(g)))),
              "unique_mcc":g.mcc.nunique(),"unique_currency":g.currency.nunique(),
              "fee_fraction":float((g.fee>0).mean()), "dom_std":g.timestamp.dt.day.std(),"dow_std":g.timestamp.dt.dayofweek.std(),
              "month_count":g.timestamp.dt.strftime("%Y-%m").nunique()}
    for window in [30,60,90,180]:
        result[f"count_{window}"] = int((days>=-window).sum())
    if len(gaps):
        result.update({"gap_median":np.median(gaps),"gap_mean":np.mean(gaps),"gap_std":np.std(gaps),"gap_min":np.min(gaps),"gap_max":np.max(gaps),
                       "gap_mad":np.median(np.abs(gaps-np.median(gaps))),"gap_cv":np.std(gaps)/max(np.mean(gaps),1e-6),
                       "next_median":days.max()+np.median(gaps),"gap_last":gaps[-1]})
...
def tabular(df, ids):
    ...
    for client_id,g in d.groupby("client_id",sort=True):
        ...
        for f in FAMILIES:
            ...
            row[f"text_{f}__refund_count"] = int(((g.direction=="in") & g["is_"+f]).sum())
        rows.append(row)
    return pd.DataFrame(rows).set_index("client_id").reindex(ids).fillna(-999).sort_index(axis=1)
