...
def transactions(split, *, use_cache=True):
    ...
    return df.sort_values(["client_id", "timestamp"],kind="stable").reset_index(drop=True)


def labels(split="train", *, allow_holdout=False):
    ...
    df = pd.read_csv(ROOT / f"data/raw/{split}_labels.csv").set_index("client_id")
    if not df.index.is_unique or df[TARGET].isna().any() or not set(df[TARGET]) <= set(LABELS):
...
def aligned_target(ids, split="train", *, allow_holdout=False):
    table = labels(split, allow_holdout=allow_holdout)
...
def validate_submission(df, sample=None):
    sample = pd.read_csv(ROOT / "data/raw/sample_submission.csv") if sample is None else sample
