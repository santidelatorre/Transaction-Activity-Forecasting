...
from .features import GENERIC, MCC, normalize
...
def canonical(text):
    text = normalize(text)
    text = (
        re.sub(r"^(?:pay|billing|member) ", "", text)
        if text not in {"member pass", "member plan"}
        else text
    )
    ...
    for f, templates in TEMPLATES.items():
        for scope in ["any", "mcc"]:
            ...
            for window in [999, 90]:
                ...
                counts = pd.crosstab(t.client_id, t.desc).reindex(
                    index=ids, columns=templates, fill_value=0
                )
                for desc in templates:
                    cols[f"{f}__{scope}_{window}_{desc}"] = counts[desc].to_numpy()
                cols[f"{f}__{scope}_{window}_diversity"] = (
                    (counts > 0).sum(axis=1).to_numpy()
                )
                cols[f"{f}__{scope}_{window}_repeated"] = (
                    (counts >= 2).sum(axis=1).to_numpy()
                )
        ...
        for desc in templates:
            ...
            for name, values in {
                ...
                "first_age": grouped.age.max(),
                "last_age": grouped.age.min(),
            }.items():
                cols[f"{f}__{desc}_{name}"] = (
                    values.reindex(ids).fillna(-999).to_numpy()
                )
