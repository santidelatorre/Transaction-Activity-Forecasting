...
def main():
    ...
    ap.add_argument("--payment-context", action="store_true")
    ap.add_argument(
        "--context-fields", choices=["all", "refund", "fee", "time"], default="all"
    )
    ap.add_argument("--sparse-candidates", action="store_true")
    ap.add_argument("--aux-expert", action="store_true")
    args = ap.parse_args()
    start = time.perf_counter()
    base = pd.read_parquet(ROOT / "data/cache/soft_ranking_original.parquet")
    ...
    for scenario in ["original", "valid_like", "test_like"]:
        ...
        rank = (
            ...
            if scenario == "original"
                ...
                ROOT / f"data/cache/soft_ranking_train_noise{scenario}_s2026.parquet"
            )
        )
        if args.sparse_candidates:
            rank = pd.read_parquet(
                ROOT / f"data/cache/sparse_ranking_{scenario}_train.parquet"
            )
        matrices[scenario] = (
            compact_features(rank, globals[scenario])
        ...
        if args.payment_context:
            path = (
                ROOT
                / f"data/cache/payment_context_{'sparse_' if args.sparse_candidates else ''}{scenario}_train.parquet"
            )
            if path.exists():
                context = pd.read_parquet(path)
            else:
                context = payment_context(d, rank)
                context.to_parquet(path)
            ...
            matrices[scenario] = pd.concat(
                [matrices[scenario], context.reindex(base.index)], axis=1
            ).fillna(-999)
        ...
        matrices[scenario].to_parquet(
            ROOT / f"data/cache/compact_{scenario}_train.parquet"
        )
    ...
    p = {k: np.zeros((len(ids), 8)) for k in matrices}
    ...
    for fold, (tr, va) in enumerate(
        ...
        X = pd.concat([m.iloc[it] for m in matrices.values()], ignore_index=True)
        yt = np.tile(target[it], 3)
        model = LGBMRanker(**params, objective="lambdarank")
        model.fit(X, yt, group=np.full(len(yt) // 8, 8))
        ...
        for key, m in matrices.items():
            ...
            if args.hierarchical:
                ...
                q[:, :7] = (
                    q[:, :7] / q[:, :7].sum(axis=1, keepdims=True) * (1 - pn[:, None])
                )
                q[:, 7] = pn
            p[key][va] = q
            scores[key].append(metrics(y[va], q)["macro_f1"])
        print("fold", fold, {k: round(s[-1], 5) for k, s in scores.items()}, flush=True)
    for scenario, prob in p.items():
        suffix = (
            ("_payments" if args.payment_context else "")
            + ("_" + args.context_fields if args.context_fields != "all" else "")
            + ("_sparse" if args.sparse_candidates else "")
            + ("_auxexpert" if args.aux_expert else "")
        )
        record(
            f"compact_l{args.leaves}_hier{args.hierarchical}{suffix}_{scenario}_s{args.seed}",
            ids,
            y,
            prob,
            runtime=time.perf_counter() - start,
            fold_scores=scores[scenario],
            metadata={
                "hypothesis": "A smaller physical recurrence representation and global transaction evidence reduce overfitting; refund patterns may distinguish continuing from terminated streams",
                "features": "Compact amount/cadence/family confidence + global type/count/activity features; three fixed noise views",
                "model": "LightGBM ranker"
                + (" + binary none" if args.hierarchical else ""),
                "parameters": params,
                "seed": args.seed,
                "protocol": "5-fold grouped client OOF; all augmented copies confined to training clients; no official labels used",
                "status": "evaluated",
                "conclusion": "Train-side response to identified generalization weakness; holdout not used for tuning",
            },
        )
