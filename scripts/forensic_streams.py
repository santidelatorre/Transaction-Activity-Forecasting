...
from ubs_recurrence.data import ROOT,CUTOFF,LABELS,transactions,labels
from ubs_recurrence.features import FAMILIES,PATTERNS,MCC,normalize,summarize,GENERIC
...
def main():
    ...
    for cid,g in df.groupby("client_id"):
        ...
        for currency,cg in card.groupby("currency"):
            cluster=DBSCAN(eps=.035,min_samples=3).fit_predict(np.log(cg.amount.to_numpy()).reshape(-1,1))
            for c in sorted(set(cluster)-{-1}):
                s=cg.iloc[np.flatnonzero(cluster==c)]
                r={"client_id":cid,"cluster":int(c),"currency":currency,"target":y.loc[cid],**summarize(s)}
                ...
                for f in FAMILIES:
                    r[f"semantic_{f}"]=s.description.str.contains(PATTERNS[f].replace("(","(?:"),regex=True).mean()
                    r[f"mcc_{f}"]=(s.mcc==MCC[f]).mean()
                rows.append(r)
        for desc,s in card.groupby("description"):
            if len(s)>=3:
                exact_rows.append({"client_id":cid,"description":desc,"target":y.loc[cid],**summarize(s)})
    ...
    scores=np.array([clusters[f"semantic_{f}"].to_numpy()+.25*clusters[f"mcc_{f}"].to_numpy() for f in FAMILIES]).T
    ...
    report=clusters.groupby(["matches_target"])[["count","amount_cv","last_age","span","unique_description","description_top_fraction","description_entropy","gap_median","gap_std","gap_cv","generic_fraction"]].agg(["mean","median"])
    report.to_csv(out/"cluster_target_relationships.csv")
    print(report.to_string())
    print("Median gap rounded distribution",clusters.gap_median.round().value_counts().head(20).to_dict())
