...
soup = BeautifulSoup((root / "data/jury_site.html").read_text(encoding="utf-8"), "html.parser")
for a in soup.find_all("a", href=True):
    if any(k in (a.get_text(" ", strip=True) + a["href"]).lower() for k in ["jury", "criteria", "evaluation", "judg"]):
        print("JURY LINK", a.get_text(" ", strip=True), a["href"])
text = soup.get_text("\n", strip=True)
(root / "data/jury_site.txt").write_text(text, encoding="utf-8")
for line in text.splitlines():
    if any(k in line.lower() for k in ["jury", "criteria", "evaluation", "agentic", "originality"]):
