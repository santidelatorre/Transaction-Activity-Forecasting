...
def main():
    ...
    for split in ["train", "valid", "test"]:
        d = transactions(split)
        ids = np.sort(d.client_id.unique())
        x = template_features(d, ids)
    ...
    for a, b in [("train", "valid"), ("train", "test")]:
        ...
        for tr, va in StratifiedKFold(5, shuffle=True, random_state=931).split(X, y):
            ...
            p[va] = model.predict_proba(X.iloc[va])[:, 1]
            scores.append(roc_auc_score(y[va], p[va]))
        ...
        report[a + ":" + b] = {
            ...
            "n_a": len(matrices[a]),
            "n_b": len(matrices[b]),
            "top_features": pd.Series(np.mean(importance, axis=0), index=X.columns)
            .sort_values(ascending=False)
            .head(15)
            .to_dict(),
        }
    y = labels()
    numeric = y.index.str[1:].astype(int)
    ct = pd.crosstab(pd.qcut(numeric, 4), y.target_next_recurring_merchant.to_numpy())
    chi, pvalue, dof, _ = chi2_contingency(ct)
    report["identifier_audit"] = {
        "quartile_class_chi_square": float(chi),
        "pvalue": float(pvalue),
        "dof": dof,
        "policy": "Identifiers and row ordering excluded regardless of association",
    }
