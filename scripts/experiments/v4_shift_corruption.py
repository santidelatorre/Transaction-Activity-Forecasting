...
def main():
    ...
    if git("branch", "--show-current") not in (BRANCH, "integration/v4-synthesis"):
        raise ValueError("Run on the assigned experiment or integration/v4-synthesis branch")
    baseline_paths = git("ls-tree", "-r", "--name-only", BASE_SHA).splitlines()
    protected = [
        name
        for name in baseline_paths
        if name in ("scripts/run_ubs_v3.py", "src/transaction_forecasting/evaluation/official.py")
        or (name.startswith("src/transaction_forecasting/ubs/") and name.endswith(".py"))
    ]
    ...
    report(result, out / "stress_report.md")
    print(
        json.dumps(
