...
def main():
    ...
    for name in args.models:
        ...
        for f, (tr, va) in enumerate(folds):
            if name in ["majority", "prior"]:
                model = DummyClassifier(
                    strategy="most_frequent" if name == "majority" else "prior"
                )
                a, b = np.zeros((len(tr), 1)), np.zeros((len(va), 1))
            elif name in ["word", "enriched", "char"]:
                vec = TfidfVectorizer(
                    analyzer="char_wb" if name == "char" else "word",
                    ngram_range=(3, 5) if name == "char" else (1, 2),
                    min_df=2,
                    sublinear_tf=True,
                    max_features=30000,
                )
        ...
        record(
            ...
            metadata={
                "hypothesis": {
                    "majority": "Measure majority-only floor",
                    "prior": "Measure empirical-prior argmax floor",
                    "word": "Description templates identify target families",
                    "char": "Character fragments recover noisy merchant wording",
                    "enriched": "MCC and text combinations disambiguate templates",
                    "tabular_lr": "Aggregated timing and family evidence is linearly useful",
                    "tabular_cat": "Nonlinear interactions among recurrence and semantic evidence improve prediction",
                }[name],
                "features": name,
                ...
                "status": "evaluated",
                "conclusion": "Exploratory baseline; compare with error analysis",
            },
        )
