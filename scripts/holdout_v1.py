...
def main():
    ...
    config = json.loads((ROOT / "reports/frozen_candidate_v1.json").read_text())
    out = ROOT / "outputs/frozen_v1"
    ...
    for kind in ["hard", "soft", "xgb", "control"]:
        ...
        if soft:
            ...
            valid = pd.read_parquet(
                ROOT / "data/cache/soft_ranking_valid_mask0.0_s2026.parquet"
            )
        else:
            train = base
            valid = pd.read_parquet(
                ROOT / "data/cache/ranking_valid_mask0.0_s2026.parquet"
            )
        ...
        if kind != "control":
            ...
            aug = [
                pd.read_parquet(
                    ROOT / f"data/cache/{prefix}_train_noise{s}_s2026.parquet"
                )
                .reindex(index=train.index, columns=train.columns)
                .fillna(-999)
                for s in ["valid_like", "test_like"]
            ]
            fit = pd.concat([train, *aug], ignore_index=True)
            yt = np.tile(ylong, 3)
        else:
            fit = train
            yt = ylong
        if kind == "xgb":
            ...
            model.fit(fit, yt, group=np.full(len(yt) // 8, 8))
        logits = model.predict(valid).reshape(-1, 8).astype(float)
        p = softmax(logits, axis=1)
        joblib.dump(
            {"model": model, "columns": train.columns.tolist(), "kind": kind},
            out / f"model_{kind}.joblib",
        )
        pred = pd.DataFrame(p, index=validation_ids, columns=LABELS)
        pred.index.name = "client_id"
        pred.to_csv(out / f"valid_{kind}.csv")
        if kind != "control":
            ps.append(p)
        else:
            control = p
    ...
    variants = {
        "control": control,
        "ensemble": ensemble,
        "calibrated": apply_bias(ensemble, config["bias"]),
    }
    for name, p in variants.items():
        pred = pd.DataFrame(p, index=validation_ids, columns=LABELS)
        pred.index.name = "client_id"
        pred.to_csv(out / f"valid_frozen_{name}.csv")
    freeze = {
        "timestamp": datetime.now(UTC).isoformat(),
        "source_sha256": source_fingerprint(),
        "selection": "Primary candidate is the calibrated equal 3-model robustness ensemble, selected using train OOF stress only. Control and uncalibrated ensemble are predeclared comparators.",
        "files": {
            p.name: hashlib.sha256(p.read_bytes()).hexdigest()
            for p in out.glob("valid*.csv")
        },
        "config": config,
    }
            ...
            json.dumps(
                {
                    "timestamp": datetime.now(UTC).isoformat(),
                    "batch": "v1",
                    "purpose": "Evaluate frozen finalists after train-only model/decision selection",
    ...
    for name, p in variants.items():
        record(
            ...
            metadata={
                "hypothesis": "Train-only robustness selection should transfer to the official distribution shift",
                "features": name,
                "model": "Frozen v1 " + name,
                "parameters": config,
                "seed": 42,
                "protocol": "Official holdout; all model predictions, ensemble weights and decision biases frozen before reading labels",
                "status": "evaluated",
                "conclusion": "First official holdout batch; compare with pre-registered train-side stress expectations",
            },
        )
