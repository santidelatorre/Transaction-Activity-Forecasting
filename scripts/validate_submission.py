...
from transaction_forecasting.evaluation.official import LABELS, PREDICTION, SUBMISSION_COLUMNS
from transaction_forecasting.evaluation.official import validate_submission as validate_contract


def inspect_submission(submission_path: Path, sample_path: Path) -> tuple[dict[str, object], bool]:
    ...
    submission_ids = submission["client_id"] if "client_id" in submission else pd.Series(dtype=str)
    sample_ids = sample["client_id"] if "client_id" in sample else pd.Series(dtype=str)
    predictions = submission[PREDICTION] if PREDICTION in submission else pd.Series(dtype=str)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--submission", type=Path, default=Path("outputs/submission_v2.csv"))
    parser.add_argument(
        "--sample",
        type=Path,
