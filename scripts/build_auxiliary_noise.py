...
from ubs_recurrence.data import CUTOFF, ROOT, transactions
...
def main():
    ...
    ap.add_argument("--severity", required=True, choices=["valid_like", "test_like"])
    args = ap.parse_args()
    ids = pd.read_csv(
        ROOT / "data/cache/auxiliary_2000_labels.csv"
    ).client_id.to_numpy()
    cutoff = pd.Timestamp("2025-10-01", tz="UTC")
    d = transactions("unlabeled_pretrain")
    d = d[d.client_id.isin(ids) & (d.timestamp < cutoff)].copy()
    d["timestamp"] += CUTOFF - cutoff
    d = corrupt_transactions(d, args.severity, 1183)
    T = template_features(d, ids)
    S = extract_streams(d, 0.035, True)
    F = family_features(S, ids)
    X = ranking_features(F, T, ids, clocks=False)
    X.to_parquet(ROOT / f"data/cache/auxiliary_2000_{args.severity}_ranking.parquet")
    print(args.severity, X.shape, flush=True)
