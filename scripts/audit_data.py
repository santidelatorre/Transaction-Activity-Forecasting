...
def main():
    ...
    if manifest != expected:
        raise ValueError(
            "Raw files differ from pinned manifest; run prepare_data.py to verify inputs"
        )
    ...
    for split in ["train", "valid", "test", "unlabeled_pretrain"]:
        ...
        df = df.sort_values(["client_id", "timestamp"], kind="stable").reset_index(
            drop=True
        )
        ...
        signatures[split] = group.apply(
            lambda g: hashlib.sha256(
                pd.util.hash_pandas_object(g, index=False).values.tobytes()
            ).hexdigest(),
        ...
        for field in ["hour", "minute", "second", "dayofweek", "day", "month"]:
            info[split]["timestamp_" + field] = (
                getattr(df.timestamp.dt, field).value_counts().sort_index().to_dict()
            )
        df.description.value_counts().rename("count").to_csv(
            OUT / f"{split}_descriptions.csv"
        )
        df.groupby(["description", "mcc", "type", "direction"]).amount.agg(
            ["count", "mean", "std", "min", "max"]
        ).to_csv(OUT / f"{split}_description_structure.csv")
        ...
        frames[split] = (
            df
            if split == "train"
            else df[["client_id", "description", "normalized_description"]]
        )
    overlaps = {}
    names = list(idsets)
    for i, a in enumerate(names):
        for b in names[i + 1 :]:
            overlaps[f"{a}:{b}"] = {
                "ids": len(idsets[a] & idsets[b]),
                "histories": len(set(signatures[a]) & set(signatures[b])),
            }
    assert all(x["ids"] == 0 for x in overlaps.values()), overlaps
    info["overlaps"] = overlaps
    train_vocab = set(frames["train"].description)
    info["unseen_description_rates"] = {
        s: float((~df.description.isin(train_vocab)).mean()) for s, df in frames.items()
    }
    ...
    for col in [
        ...
        for unit, rows in [
            ("client", tr.drop_duplicates(["client_id", col])),
            ("event", tr),
        ]:
            table = pd.crosstab(rows[col], rows[target])
            table.to_csv(OUT / f"target_by_{col}_{unit}_counts.csv")
            table.div(table.sum(axis=1), axis=0).to_csv(
                OUT / f"target_by_{col}_{unit}_prob.csv"
            )
    ...
    numeric = tr.groupby("client_id").agg(
        n=("amount", "size"),
    ...
    numeric["duration"] = (
        numeric["last"] - numeric["first"]
    ).dt.total_seconds() / 86400
    numeric.join(labels[target]).groupby(target).describe().to_csv(
        OUT / "client_statistics_by_target.csv"
    )
