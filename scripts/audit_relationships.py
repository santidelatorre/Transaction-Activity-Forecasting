...
from ubs_recurrence.data import CUTOFF, LABELS, ROOT, TARGET, labels, transactions
...
def conditional(rows, key, name, out):
    ...
    table = pd.crosstab(unique[key], unique[TARGET]).reindex(
...
def main():
    ...
    d = transactions("train", use_cache=False).join(
        labels()[TARGET], on="client_id", validate="many_to_one"
    )
    ...
    d["amount_band"] = pd.cut(
        d.amount, [0, 5, 10, 20, 40, 80, 160, 320, 1000, np.inf], include_lowest=True
    ).astype(str)
    ...
    dominant = (
        eligible.div(eligible.sum(axis=1), axis=0)
        .max(axis=1)
        .sort_values(ascending=False)
    )
    rows = []
    card = d[(d.type == "card_payment") & (d.direction == "out")]
    for (cid, description), group in card.groupby(
        ["client_id", "normalized_description"]
    ):
        if len(group) < 2:
            continue
        ...
        rows.append(
            {
                ...
                "count": len(group),
                "recency_days": (CUTOFF - group.timestamp.max()).total_seconds()
                / 86400,
                "span_days": (
                    group.timestamp.max() - group.timestamp.min()
                ).total_seconds()
                / 86400,
                "median_interval_days": gap.median(),
                "interval_std_days": gap.std(ddof=0),
                "amount_mean": group.amount.mean(),
                "amount_std": group.amount.std(ddof=0),
            }
        )
    streams = pd.DataFrame(rows)
    streams["interval_cv"] = (
        streams.interval_std_days / streams.median_interval_days.clip(lower=0.01)
    )
    streams["occurrence_band"] = pd.cut(
        streams["count"], [1, 2, 4, 8, 16, np.inf]
    ).astype(str)
    streams["recency_band"] = pd.cut(
        streams.recency_days, [0, 15, 30, 60, 90, 180, np.inf]
    ).astype(str)
    streams["interval_band"] = pd.cut(
        streams.median_interval_days, [0, 10, 20, 35, 65, 100, 200, np.inf]
    ).astype(str)
    streams["regularity_band"] = pd.cut(
        streams.interval_cv, [-0.001, 0.1, 0.25, 0.5, 1, np.inf]
    ).astype(str)
    ...
    ].agg(["count", "mean", "median", "std"]).to_csv(
        out / "stream_statistics_by_target.csv"
    )
    report = {
        "scope": "Train labels only; descriptive, not predictive validation; produced after model freeze",
        ...
        "most_concentrated_ngrams_minimum_50_clients": [
            {
                "ngram": n,
                "clients": int(count[n]),
                "dominant_label": eligible.loc[n].idxmax(),
                "dominant_fraction": float(dominant[n]),
            }
            for n in dominant.head(12).index
        ],
        "warning": "Conditional association is not a classifier score; background exposure and class priors are confounders.",
    }
    (ROOT / "reports/target_relationships.json").write_text(
